...
func (h *DesksHandler) ChangeDeskDate(w http.ResponseWriter, r *http.Request) {
	h.log.Info("new request", slog.String("path", "/desks/{id}/update"))
	...
	if !ok {
		h.log.Error("change desk data failed: error get desk id in path")
	...
	if err != nil {
		h.log.Error("change desk data failed: error parsing desk id", slog.Any("err", err))
	...
	if err != nil {
		h.log.Error("change desk data failed: error parsing user id",
			slog.Any("deskID", deskUUID), slog.Any("err", err))
