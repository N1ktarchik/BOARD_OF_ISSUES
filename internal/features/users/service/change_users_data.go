...
func (s *UsersService) ChangeUsersData(ctx context.Context, user *domain.User) (*domain.User, error) {
	...
	if user.Name == "" && user.Email == "" && user.Password == "" {
		s.log.Error("change user data successfully : no data to change", slog.String("user_id", user.ID.String()))
		return user, nil
	}
	...
	if err != nil {
		s.log.Error("change users data failed : password hashing error", slog.Any("err", err),
